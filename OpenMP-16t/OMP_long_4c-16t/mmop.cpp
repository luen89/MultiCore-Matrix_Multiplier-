...
int main (int argc, char *argv[]) {
  ...
  long** a=(long**) malloc(orden*sizeof(long*));
  for(int l=0;l<orden;l++)
  	a[l]=(long*) malloc(orden*sizeof(long));

  long** b=(long**) malloc(orden*sizeof(long*));
  for(int l=0;l<orden;l++)
  	b[l]=(long*) malloc(orden*sizeof(long));

  long** c=(long**) malloc(orden*sizeof(long*));
  for(int l=0;l<orden;l++)
  c[l]=(long*) malloc(orden*sizeof(long));


  chunk = 10;  
  for (i=0; i<NRA; i++)
    for (j=0; j<NCA; j++)
      a[i][j]= (long)(1000 + rand() % (2000-1000+1));
  for (i=0; i<NCA; i++)
    for (j=0; j<NCB; j++)
      b[i][j]= (long)(1000 + rand() % (2000-1000+1));
  for (i=0; i<NRA; i++)
    for (j=0; j<NCB; j++)
