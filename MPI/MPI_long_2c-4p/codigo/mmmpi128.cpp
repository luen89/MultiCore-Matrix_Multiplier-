...
long first_matrix[MATRIX_SIZE][MATRIX_SIZE];
long second_matrix[MATRIX_SIZE][MATRIX_SIZE];
...
void MultiplyLocal(long **a, long **b, long **c, int size) {
    int temp = 0;
...
void UnpackMatrix(long *buff, long **a, int size) {
    int k = 0;
    for (int i = 0; i < size; i++)
        for (int j = 0; j < size; j++) {
            a[i][j] = buff[k];
...
void PackMatrix(long *buff, long **a, int size) {
    int k = 0;
    for (int i = 0; i < size; i++)
        for (int j = 0; j < size; j++) {
            buff[k] = a[i][j];
...
void PrintPackedMatrix(long *matrix, int size) {
    for (int i = 0; i < size; i++) {
       // std::cout << "|";
        for (int j = 0; j < size; j++) {
            long el = matrix[i * size + j];
            if (el < 10)
                std::cout << " ";
            std::cout << el;
            std::cout << "|";
    ...
    for (int i = 0; i < MATRIX_SIZE; i++){
      //std::cout << "\n";
      	...
      	for (int j = 0; j < MATRIX_SIZE; j++) {
            ...
            first_matrix[i][j] = (long)(1000 + rand() % (2000-1000+1));//(i==j) ? 1 : 0;
            //std::cout << first_matrix[i][j];
            //std::cout << "__";
            second_matrix[i][j] = (long)(1000 + rand() % (2000-1000+1));
...
void FoxMultiply(int n, GridStructure *grid, long **a, long **b, long **c) {
    long **temp_a, *buff;
    ...
    temp_a = new long*[submat_dim];
    for(int i = 0; i < submat_dim; ++i)
        temp_a[i] = new long[submat_dim];
    for (int i = 0; i < submat_dim; i++)
        for (int j = 0; j < submat_dim; j++)
            temp_a[i][j] = 0;

    buff = new long[submat_dim*submat_dim];
...
long** TestSingleThread()
{
    long **test_result, **test_a, **test_b;
    test_a = new long*[MATRIX_SIZE];
    test_b = new long*[MATRIX_SIZE];
    test_result = new long*[MATRIX_SIZE];
    for(int i = 0; i < MATRIX_SIZE; ++i) {
        test_a[i] = new long[MATRIX_SIZE];
        test_b[i] = new long[MATRIX_SIZE];
        test_result[i] = new long[MATRIX_SIZE];
    }
    for (int i = 0; i < MATRIX_SIZE; i++)
        for (int j = 0; j < MATRIX_SIZE; j++) {
...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    local_a = new long*[MATRIX_SIZE];
    local_b = new long*[MATRIX_SIZE];
    local_c = new long*[MATRIX_SIZE];
    for(int i = 0; i < MATRIX_SIZE; ++i)
    {
        local_a[i] = new long[MATRIX_SIZE];
        local_b[i] = new long[MATRIX_SIZE];
        local_c[i] = new long[MATRIX_SIZE];
    }
    ...
    long *result_buff = new long[MATRIX_SIZE * MATRIX_SIZE];
    long *local_buff = new long[block_size * block_size];
    ...
    MPI_Barrier(grid.grid_comm);
    if (grid.rank == 0) {
        long *data = new long[MATRIX_SIZE * MATRIX_SIZE];
        int k = 0;
