...
float first_matrix[MATRIX_SIZE][MATRIX_SIZE];
float second_matrix[MATRIX_SIZE][MATRIX_SIZE];

//Funcion que retorna el uso de memoria 
...
//Struct que contiene toda la información para cada proceso
typedef struct {
    int proc_count;
    int dim;
    int row;
...
void GridSetup(GridStructure *grid) {
    ...
    //Se calcula cuantos procesos por dimension
    ...
    //Vector lógico
    //Si es 1, se mantiene. Si es 0, se libera
    //Cambio de columna
    free_coords[0] = 1;
    free_coords[1] = 0;
...
void MultiplyLocal(float **a, float **b, float **c, int size) {
    int temp = 0;
...
void UnpackMatrix(float *buff, float **a, int size) {
    int k = 0;
    for (int i = 0; i < size; i++)
        for (int j = 0; j < size; j++) {
            a[i][j] = buff[k];
...
void PackMatrix(float *buff, float **a, int size) {
    int k = 0;
    for (int i = 0; i < size; i++)
        for (int j = 0; j < size; j++) {
            buff[k] = a[i][j];
...
void PrintPackedMatrix(float *matrix, int size) {
    for (int i = 0; i < size; i++) {       
        for (int j = 0; j < size; j++) {
            float el = matrix[i * size + j];
            if (el < 10)
                std::cout << " ";
            std::cout << el;
            std::cout << "|";
...
//Se crean las matrices y se rellenan con numeros entre 1000 y 2000
void GenerateMatrices() {
	srand(time(NULL));
    for (int i = 0; i < MATRIX_SIZE; i++){
      	for (int j = 0; j < MATRIX_SIZE; j++) {
            first_matrix[i][j] = (float)(1000 + rand() % (2000-1000+1));
            second_matrix[i][j] = (float)(1000 + rand() % (2000-1000+1));
        }
    }
}

//Ocurre la multiplicacion de matrices con el algortimo de fox
void FoxMultiply(int n, GridStructure *grid, float **a, float **b, float **c) {
    float **temp_a, *buff;
    int stage, root, submat_dim, src, dst;
    MPI_Status status;

    submat_dim = n / grid->dim;


    temp_a = new float*[submat_dim];
    for(int i = 0; i < submat_dim; ++i)
        temp_a[i] = new float[submat_dim];
    for (int i = 0; i < submat_dim; i++)
        for (int j = 0; j < submat_dim; j++)
            temp_a[i][j] = 0;

    buff = new float[submat_dim*submat_dim];
    ...
    //n etapas para matrices de orden n
    for (stage = 0; stage < grid->dim; stage++) {
        root = (grid->row + stage) % grid->dim;
        if (root == grid->col) {
            PackMatrix(buff, a, submat_dim);
            MPI_Bcast(buff, submat_dim * submat_dim, MPI_INT, root, grid->row_comm); //broadcast
...
float** TestSingleThread()
{
    float **test_result, **test_a, **test_b;
    test_a = new float*[MATRIX_SIZE];
    test_b = new float*[MATRIX_SIZE];
    test_result = new float*[MATRIX_SIZE];
    for(int i = 0; i < MATRIX_SIZE; ++i) {
        test_a[i] = new float[MATRIX_SIZE];
        test_b[i] = new float[MATRIX_SIZE];
        test_result[i] = new float[MATRIX_SIZE];
    }
    for (int i = 0; i < MATRIX_SIZE; i++)
        for (int j = 0; j < MATRIX_SIZE; j++) {
...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    local_a = new float*[MATRIX_SIZE];
    local_b = new float*[MATRIX_SIZE];
    local_c = new float*[MATRIX_SIZE];
    for(int i = 0; i < MATRIX_SIZE; ++i)
    {
        local_a[i] = new float[MATRIX_SIZE];
        local_b[i] = new float[MATRIX_SIZE];
        local_c[i] = new float[MATRIX_SIZE];
    }
    ...
    MPI_Barrier(grid.grid_comm); //Se espera a que todos los procesos terminen
    ...
    float *result_buff = new float[MATRIX_SIZE * MATRIX_SIZE];
    float *local_buff = new float[block_size * block_size];
    ...
    MPI_Barrier(grid.grid_comm);  //Se espera a que todos los procesos terminen
    if (grid.rank == 0) {
        float *data = new float[MATRIX_SIZE * MATRIX_SIZE];
        int k = 0;
