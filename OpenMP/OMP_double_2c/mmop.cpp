...
int main (int argc, char *argv[]) {
  ...
  double** a=(double**) malloc(orden*sizeof(double*));
  for(int l=0;l<orden;l++)
  	a[l]=(double*) malloc(orden*sizeof(double));

  double** b=(double**) malloc(orden*sizeof(double*));
  for(int l=0;l<orden;l++)
  	b[l]=(double*) malloc(orden*sizeof(double));

  double** c=(double**) malloc(orden*sizeof(double*));
  for(int l=0;l<orden;l++)
  c[l]=(double*) malloc(orden*sizeof(double));


  chunk = 10;  
  for (i=0; i<NRA; i++)
    for (j=0; j<NCA; j++)
      a[i][j]= (double)(1000 + rand() % (2000-1000+1));
  for (i=0; i<NCA; i++)
    for (j=0; j<NCB; j++)
      b[i][j]= (double)(1000 + rand() % (2000-1000+1));
  for (i=0; i<NRA; i++)
    for (j=0; j<NCB; j++)
