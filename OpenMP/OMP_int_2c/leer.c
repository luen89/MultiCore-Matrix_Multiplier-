...
int main ( int argc, char **argv ){
    ...
    char time[35];
    snprintf(time, sizeof(time), "archivos/tiempo/TO%d.txt",rango);
    char preTime[35];
    snprintf(preTime, sizeof(preTime), "archivos/tiempo/TOPre%d.txt",rango);
    char promTP[35];
    snprintf(promTP, sizeof(promTP), "archivos/tiempo/TOProm%d.txt",rango);

    char mem[35];
    snprintf(mem, sizeof(mem), "archivos/memoria/MO%d.txt",rango);
    char preMem[35];
    snprintf(preMem, sizeof(preMem), "archivos/memoria/MOPre%d.txt",rango);
    char promMP[35];
    snprintf(promMP, sizeof(promMP), "archivos/memoria/MOProm%d.txt",rango);

 	fPreTime = fopen ( preTime, "r" );
    fTime = fopen ( time, "a" );
    fTPromA = fopen ( "archivos/tiempo/TOProm.txt", "a" );
    fTPromP = fopen ( promTP, "a");

    fPreMem = fopen ( preMem, "r" );
    fMem= fopen ( mem, "a" );
    fMPromA = fopen ( "archivos/memoria/MOProm.txt", "a" );
