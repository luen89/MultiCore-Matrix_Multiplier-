...
double first_matrix[MATRIX_SIZE][MATRIX_SIZE];
double second_matrix[MATRIX_SIZE][MATRIX_SIZE];
...
void MultiplyLocal(double **a, double **b, double **c, int size) {
    int temp = 0;
...
void UnpackMatrix(double *buff, double **a, int size) {
    int k = 0;
    for (int i = 0; i < size; i++)
        for (int j = 0; j < size; j++) {
            a[i][j] = buff[k];
...
void PackMatrix(double *buff, double **a, int size) {
    int k = 0;
    for (int i = 0; i < size; i++)
        for (int j = 0; j < size; j++) {
            buff[k] = a[i][j];
...
void PrintPackedMatrix(double *matrix, int size) {
    for (int i = 0; i < size; i++) {
       // std::cout << "|";
        for (int j = 0; j < size; j++) {
            double el = matrix[i * size + j];
            if (el < 10)
                std::cout << " ";
            std::cout << el;
            std::cout << "|";
    ...
    for (int i = 0; i < MATRIX_SIZE; i++){
      //std::cout << "\n";
      	...
      	for (int j = 0; j < MATRIX_SIZE; j++) {
            ...
            first_matrix[i][j] = (double)(1000 + rand() % (2000-1000+1));//(i==j) ? 1 : 0;
            //std::cout << first_matrix[i][j];
            //std::cout << "__";
            second_matrix[i][j] = (double)(1000 + rand() % (2000-1000+1));
...
void FoxMultiply(int n, GridStructure *grid, double **a, double **b, double **c) {
    double **temp_a, *buff;
    ...
    temp_a = new double*[submat_dim];
    for(int i = 0; i < submat_dim; ++i)
        temp_a[i] = new double[submat_dim];
    for (int i = 0; i < submat_dim; i++)
        for (int j = 0; j < submat_dim; j++)
            temp_a[i][j] = 0;

    buff = new double[submat_dim*submat_dim];
...
double** TestSingleThread()
{
    double **test_result, **test_a, **test_b;
    test_a = new double*[MATRIX_SIZE];
    test_b = new double*[MATRIX_SIZE];
    test_result = new double*[MATRIX_SIZE];
    for(int i = 0; i < MATRIX_SIZE; ++i) {
        test_a[i] = new double[MATRIX_SIZE];
        test_b[i] = new double[MATRIX_SIZE];
        test_result[i] = new double[MATRIX_SIZE];
    }
    for (int i = 0; i < MATRIX_SIZE; i++)
        for (int j = 0; j < MATRIX_SIZE; j++) {
...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    local_a = new double*[MATRIX_SIZE];
    local_b = new double*[MATRIX_SIZE];
    local_c = new double*[MATRIX_SIZE];
    for(int i = 0; i < MATRIX_SIZE; ++i)
    {
        local_a[i] = new double[MATRIX_SIZE];
        local_b[i] = new double[MATRIX_SIZE];
        local_c[i] = new double[MATRIX_SIZE];
    }
    ...
    double *result_buff = new double[MATRIX_SIZE * MATRIX_SIZE];
    double *local_buff = new double[block_size * block_size];
