...
int main ( int argc, char **argv ){
    ...
    char time[35];
    snprintf(time, sizeof(time), "archivos/tiempo/TS%d.txt",rango);
    char preTime[35];
    snprintf(preTime, sizeof(preTime), "archivos/tiempo/TSPre%d.txt",rango);
    char promTP[35];
    snprintf(promTP, sizeof(promTP), "archivos/tiempo/TSProm%d.txt",rango);

    char mem[35];
    snprintf(mem, sizeof(mem), "archivos/memoria/MS%d.txt",rango);
    char preMem[35];
    snprintf(preMem, sizeof(preMem), "archivos/memoria/MSPre%d.txt",rango);
    char promMP[35];
    snprintf(promMP, sizeof(promMP), "archivos/memoria/MSProm%d.txt",rango);

 	fPreTime = fopen ( preTime, "r" );
    fTime = fopen ( time, "a" );
    fTPromA = fopen ( "archivos/tiempo/TSProm.txt", "a" );
    fTPromP = fopen ( promTP, "a");

    fPreMem = fopen ( preMem, "r" );
    fMem= fopen ( mem, "a" );
    fMPromA = fopen ( "archivos/memoria/MSProm.txt", "a" );
